...
def user_management_controller_test (email, user_management_controller, user_id):
    ...
    user = user_management_controller.user_management_create_user(register_user_request);
...
def test_user_purchase (products_controller, client_id, client_secret, store_id, username, password):
    ...
    user_management_controller = UserManagementController(client_id, client_secret);
    user_stores_controller = UserStoresController(client_id, client_secret);
    user_purchases_controller = UserPurchasesController(client_id, client_secret);
    ...
    user_store = user_stores_controller.user_stores_connect_store(store_connect, user_id).result;
    ...
    user_products = products_controller.products_get_user_products(user_id, 1, 15, True, True).result;
    ...
    barcode_response = user_scans_controller.user_scans_upload_barcode(barcode_request, user_id).result;
...
def lookup_controller_test (lookup_controller, store_name):
    ...
    if (alternatives.__len__() == 0 or alternatives[0].id != 1 or alternatives[0].name != "Reduce Sodium"):
    ...
    if (units_of_measurement.__len__() == 0 or units_of_measurement[0].id != 1 or units_of_measurement[0].name != "g"):
        raise APITestException("Error: units of measurements")

    stores = lookup_controller.lookup_get_stores().result;
    if (stores.__len__() == 0 or stores[0].id != 1 or stores[0].name != "FreshDirect"):
...
def products_controller_test (products_controller):
    ...
    kale_products = products_controller.products_search_products("Kale", None, 1, 25, None, True).result;
    ...
    second_page_kale_products = products_controller.products_search_products("Kale", None, 2, 25, None, True).result;
    if (second_page_kale_products.__len__() == 0 or second_page_kale_products[0].name is None or second_page_kale_products[0].id == kale_products[0].id):
        raise APITestException("Error: get 2nd page products")

    upc_product = products_controller.products_search_products(None, "014100044208", 1, 25, None, True).result;
    if (upc_product.__len__() == 0 or upc_product[0].name != "Pepperidge Farm Classic Bbq Cracker Chips, 6 Oz"):
        raise APITestException("Error: get upc products")

    ean_product = products_controller.products_search_products(None, "096619872404", None, None, None, True).result;
    if (ean_product.__len__() == 0 or ean_product[0].name != "Beckett Basketball Monthly Houston Rocket English"):
        raise APITestException("Error: get ean products")

    product_full = products_controller.products_get_product("380728", True).result;
    if (product_full is None or product_full.name != "Peanut Butter Chocolate Party Size Candies"):
        raise APITestException("Error: get full product")

    products_alternatives = products_controller.products_get_products_alternatives("120907, 120902", "7").result;
    if (products_alternatives.__len__() == 0 or products_alternatives[0].product_alternatives[0].name != "Lightlife Smart Deli Veggie Slices Roast Turkey"):
        raise APITestException("Error: get full product")

    product_prices = products_controller.products_get_product_prices("149109, 113427").result;
    if (product_prices.__len__() == 0 or product_prices[0].prices[0].store_id != 4):
        raise APITestException("Error: get full product")
...
try:
    ...
    store_id = lookup_controller_test(LookupController(client_id, client_secret), store_name)
